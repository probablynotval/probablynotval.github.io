...
import { cartesian, color, fmt, isThemeDark, mapObjectValues, math, polar, prefersReducedMotion, toCartesian, } from "../utils/index.js";
        ...
        hooks: {
            ...
            draw: ({ ctx, data, dt, elapsedTime, layout, input }) => {
                ...
                for (const [i, { startAngle, endAngle }] of angles.entries()) {
                    ...
                    const offsetEnd = math.clamp(normEnd + Math.PI / 2, 0, Math.PI * 2 - 0.000000000000001) % (2 * Math.PI);
                    if (dist > radius) {
                        continue;
                    }
                ...
                for (const [i, { startAngle, endAngle, id }] of angles.entries()) {
                    ...
                    if (prevAngles !== null) {
                        ...
                        if (prev !== undefined) {
                            ...
                            // @ts-expect-error: known to be defined
            ...
            buildTable: ({ data, table, tableSummary }) => {
                ...
                const companyNames = data
                    ...
                    .slice(1)
                    .map((x) => x.trim());
                const rawData = data.getRowsTransposed("white", "asian", "latino", "black", "multi", "other");
