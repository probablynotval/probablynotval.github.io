...
import { cartesian, color, draw, findHoveredPoint, fmt, isThemeDark, math, prefersReducedMotion, shuffleArray, } from "../utils/index.js";
...
export const payGapByJob = (parent, dimensions, padding, animationCycle) => {
    ...
    const dotSizeMin = 15;
    const dotSizeMax = 40;
    const proportionFemaleToWidth = (value, layout) => math.map(value, propFemaleMin, propFemaleMax, layout.leftEdge, layout.rightEdge);
    const payGapToHeight = (value, layout) => math.map(value, payGapMax, payGapMin, layout.topEdge + dotSizeMax / 2, layout.bottomEdge - dotSizeMax / 2);
    const allData = data
        .getRows("job_type_code", "job_subtype", "num_jobs_female", "num_jobs", "proportion_female", "pay_gap")
        .map((d) => ({
        ...
        hooks: {
            preload: ({ layout }) => {
                for (const [id, col] of allData.entries()) {
                    const radius = (math.map(col.numJobs, numJobsMin, numJobsMax, dotSizeMin, dotSizeMax) * 1.2) / 2;
            ...
            draw: ({ canvas, ctx, dt, elapsedTime, layout, input }) => {
                ...
                draw.axis(ctx, { ...layout, xTickLabelCount, yTickLabelCount }, { elapsedTime, duration: axisDuration / 5 });
                ctx.lineWidth = 1;
                ctx.strokeStyle = color.oklch(COLOR.surface1());
                drawCenterAxes(ctx, layout, { elapsedTime, duration: axisDuration });
                ctx.restore();
                ctx.save();
                draw.yAxisTickLabels(ctx, { ...layout, padding, xTickLabelCount, yTickLabelCount }, payGapMax, payGapMin, payGapToHeight, { elapsedTime, duration: axisDuration / 5, inverse: true });
                draw.xAxisTickLabels(ctx, { ...layout, padding }, [0, 25, 50, 75, 100], proportionFemaleToWidth, {
                    elapsedTime,
                ...
                const { x, y, radius, dataColor, proportionFemale, payGap, numJobs, numJobsFemale } = hoveredPoint;
            ...
            buildTable: ({ data, table, tableSummary }) => {
                ...
                const rawData = data.getRows("job_subtype", "num_jobs_female", "num_jobs", "proportion_female", "pay_gap");
