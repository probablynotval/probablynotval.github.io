...
import { cartesian, color, draw, findHoveredPoint, fmt, forcedColors, isThemeDark, math, prefersReducedMotion, } from "../utils/index.js";
...
export const climateChange = (parent, dimensions, padding, animationCycle) => {
    ...
    const yearToWidth = (value, layout) => math.map(value, sliderStart, sliderEnd, layout.leftEdge, layout.rightEdge);
    const widthToYear = (value, layout) => math.map(value, layout.leftEdge, layout.rightEdge, sliderStart, sliderEnd);
    const tempToHeight = (value, layout) => math.map(value, minTemperature, maxTemperature, layout.bottomEdge, layout.topEdge);
    const heightToTemp = (value, layout) => math.map(value, layout.bottomEdge, layout.topEdge, minTemperature, maxTemperature);
    ...
    const updateSliderGradient = (progress) => {
        ...
        const colors = Array.from(temps.slice(start, end), (t, i) => (i + 1) / (end - start) <= progress
    ...
    const updateSliderOnInput = () => yearSlider.set([Number(sliderInputMin.value), Number(sliderInputMax.value)]);
    let resizeObserver;
    let connectSegment;
        ...
        hooks: {
            ...
            setup: ({ canvas, layout }) => {
                ...
                resizeObserver = new ResizeObserver(() => {
                    ...
                    sliderContainer.style.setProperty("--padding-right", `${(canvas.width - layout.rightEdge) * scale}px`);
            ...
            draw: ({ canvas, ctx, elapsedTime, dt, data, input, layout }) => {
                ...
                draw.axisLabels(ctx, { ...layout, canvasWidth: canvas.width, canvasHeight: canvas.height }, xAxisLabel, yAxisLabel, { elapsedTime: labelTime, duration: animationCycle * 5 });
                ctx.save();
                ...
                ctx.restore();
                ctx.save();
                draw.yAxisTickLabels(ctx, { ...layout, padding, xTickLabelCount, yTickLabelCount }, minTemperature, maxTemperature, tempToHeight, { elapsedTime, duration: animationCycle }, 1);
