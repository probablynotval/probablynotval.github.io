// biome-ignore-all lint/style/noNonNullAssertion: because I said so
import { Chart } from "../chart.js";
import { loadCsvData } from "../csv.js";
import { COLOR } from "../utils/color.js";
import { cartesian, color, draw, findHoveredPoint, fmt, isThemeDark, math, prefersReducedMotion, } from "../utils/index.js";
...
export const payGap = (parent, dimensions, padding, animationCycle) => {
    ...
    const pointRadius = 5;
    const minPayGap = 0;
    const yearToWidth = (value, layout) => math.map(value, startYear, endYear, layout.leftEdge, layout.rightEdge);
    const widthToYear = (value, layout) => math.map(value, layout.leftEdge, layout.rightEdge, startYear, endYear);
    const payGapToHeight = (value, layout) => math.map(value, minPayGap, maxPayGap, layout.bottomEdge, layout.topEdge);
    const heightToPayGap = (value, layout) => math.map(value, layout.bottomEdge, layout.topEdge, minPayGap, maxPayGap);
    ...
    return new Chart(parent, {
        ...
        hooks: {
            ...
            draw: ({ canvas, ctx, data, dt, elapsedTime, layout, input }) => {
                ...
                draw.axisLabels(ctx, { ...layout, canvasWidth: canvas.width, canvasHeight: canvas.height }, xAxisLabel, yAxisLabel, { elapsedTime: labelTime, duration: animationCycle });
                // Don't draw under the main axis.
                const yearTickValues = years.slice(1);
                ctx.strokeStyle = color.oklch(COLOR.surface1());
                draw.yAxisGrid(ctx, { ...layout, xTickLabelCount, yTickLabelCount }, minPayGap, maxPayGap, payGapToHeight, { elapsedTime, duration: animationCycle });
                draw.xAxisGrid(ctx, layout, yearTickValues, [COLOR.surface1(), COLOR.surface0()], yearToWidth, {
                    elapsedTime: xGridTime,
                    duration: animationCycle,
                });
                ctx.save();
                ctx.strokeStyle = color.oklch(COLOR.text());
                draw.axis(ctx, { ...layout, xTickLabelCount, yTickLabelCount }, { elapsedTime, duration: animationCycle });
                ctx.restore();
                ctx.save();
                draw.yAxisTickLabels(ctx, { ...layout, padding, xTickLabelCount, yTickLabelCount }, minPayGap, maxPayGap, payGapToHeight, { elapsedTime, duration: animationCycle });
                draw.xAxisTickLabels(ctx, { ...layout, padding }, data.getColumn("year").map(Number), yearToWidth, { elapsedTime, duration: animationCycle });
                ctx.restore();
