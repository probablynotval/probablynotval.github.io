...
export const map = (x, start0, stop0, start1, stop1) =>
  ((x - start0) / (stop0 - start0)) * (stop1 - start1) + start1;
...
export const easeOutElastic = (x, period = 0.3, decay = 10) =>
  x === 0 ? 0 : x === 1 ? 1 : 2 ** (-decay * x) * Math.sin((x * (2 * Math.PI)) / period - 1) + 1;
