...
export const yAxisGrid = (ctx, layout, min, max, mapFn, animation) => {
  ...
  const mappedValues = Array.from({ length: n }, (_, i) =>
    mapFn(min + i * ((max - min) / (n - 1)), layout),
  );
...
export const xAxisTickLabels = (ctx, layout, values, mapFn, animation, rotation = 30) => {
  ...
  for (const [i, value] of values.entries()) {
    ...
    const offCenter = 4;
    const xOffset =
      rotation === 0 ? 0 : rotation < 0 ? -textWidth / offCenter : textWidth / offCenter;
