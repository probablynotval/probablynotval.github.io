...
export class Chart {
    ...
    constructor(parent, opts) {
        ...
        this.#parent = parent;
        this.#options = opts;
        this.#layout = this.#computeLayout(ctx, opts.dimensions, opts.offsets ?? { left: 0, right: 0, top: 0, bottom: 0 }, opts.title, opts.titleSize);
    ...
    #computeLayout = (ctx, dimensions, { left = 0, right = 0, top = 0, bottom = 0 }, title, titleSize) => {
...
class ChartInput {
    ...
    isMouseInPlot = (offset = 0) => {
        ...
        return (this.#mousePos.x > this.#layout.leftEdge + left &&
...
class ChartTooltip {
    ...
    draw(ctx, layout, timekeeper, { hoveredPoint, drawnPoints, title, dataMap, mouseInPlot, marginFromPoint, animationDuration, strokeColor, offset, titleFontSize = 16, fontSize = 14, }) {
        ...
        if (hoveredPoint === null) {
            console.warn("hoveredPoint is null - expected a non-null value (caller should guard against this)");
            return;
        }
        ...
        ctx.restore();
        const rectMargin = typeof marginFromPoint === "number"
        ...
        const finalTarget = cartesian(this.#target.x + xOffset - xOffsetCenter, this.#target.y + yOffset - yOffsetCenter);
        const tooltip = {
            ctx,
            title: String(title),
            pos: finalTarget,
